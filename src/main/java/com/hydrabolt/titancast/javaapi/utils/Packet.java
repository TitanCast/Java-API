...
public class Packet {
    ...
    public Packet( String packetType, String packetData ) {
        this.type = packetType;
        this.data = new ArrayList<String>( Arrays.asList( new String[]{
                packetData
        } ) );
    }
    ...
    public Packet( String data ) {
        ...
        for (int iteration = 0; iteration < chunks.length; iteration++) {
            //this entire for loop is a MESS! need to fix this soon.
            if (iteration == 0) {
                this.type = chunks[0];
                continue;
            }
            try {
                dataChunks[iteration - 1] = new String( Base64.decode( chunks[iteration].getBytes() ), "UTF-8" );
            } catch (IOException e) {
                dataChunks[iteration - 1] = "";
                e.printStackTrace();
                System.out.println( chunks[iteration] );
            }
